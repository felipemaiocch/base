...
function chunkMatchesPhrase(chunk, phrase) {
  if (!phrase) {
    return false;
  }
  ...
  return sectionTitle.includes(phrase) || searchableText.includes(phrase);
}

function buildTermFrequencies(chunks, terms) {
  const frequencies = {};
  ...
  for (const term of terms) {
    ...
    for (const chunk of chunks) {
      ...
      if (sectionTitle.includes(term) || searchableText.includes(term)) {
        count += 1;
      }
    }
...
function resolveAnchorPhrase(chunks, phrases) {
  const candidates = phrases
    .filter((phrase) => phrase.split(' ').length >= 2)
    .map((phrase) => ({
      phrase,
      frequency: chunks.filter((chunk) => chunkMatchesPhrase(chunk, phrase)).length,
    }))
...
function resolveTermWeight(term, termFrequencies, totalChunks) {
  const frequency = termFrequencies[term] || 0;
  if (frequency === 0 || totalChunks === 0) {
    return 1;
  }

  const ratio = frequency / totalChunks;
...
function chunkMatchesAnyTerm(chunk, terms) {
  ...
  return terms.some((term) => sectionTitle.includes(term) || searchableText.includes(term));
}

function scoreChunk(chunk, terms, phrases, termFrequencies, totalChunks) {
  const sectionTitle = normalizeForSearch(chunk.section_title || chunk.sectionTitle || '');
  const searchableText = chunk.searchable_text || chunk.searchableText || normalizeForSearch(chunk.content || '');
  ...
  for (const term of terms) {
    ...
    const termWeight = resolveTermWeight(term, termFrequencies, totalChunks);

    if (titleHits > 0) {
      score += Math.round((16 + Math.min(titleHits, 2) * 5) * termWeight);
    }

    if (contentHits > 0) {
      score += Math.round(Math.min(contentHits, 6) * Math.min(term.length, 8) * termWeight);
    }
  }
...
function selectRelevantKnowledgeChunks(chunks, message, history) {
  ...
  const anchorPhrase = resolveAnchorPhrase(chunks, phrases);
  const termFrequencies = buildTermFrequencies(chunks, terms);
  const positiveFrequencies = terms
    .map((term) => ({
      term,
      frequency: termFrequencies[term] || 0,
    }))
    .filter((item) => item.frequency > 0);
  const rarestFrequency = positiveFrequencies.length > 0 ? Math.min(...positiveFrequencies.map((item) => item.frequency)) : 0;
  const anchorTerms =
    rarestFrequency > 0 && rarestFrequency / chunks.length <= 0.2
      ? positiveFrequencies.filter((item) => item.frequency === rarestFrequency).map((item) => item.term)
      : [];

  const scoredChunks = chunks
    .map((chunk) => ({
      chunk,
      score: scoreChunk(chunk, terms, phrases, termFrequencies, chunks.length),
    }))
  ...
  const anchoredChunks =
    anchorPhrase
      ? scoredChunks.filter((item) => chunkMatchesPhrase(item.chunk, anchorPhrase))
      : anchorTerms.length > 0
      ? scoredChunks.filter((item) => chunkMatchesAnyTerm(item.chunk, anchorTerms))
      : scoredChunks;
  const candidateChunks = anchoredChunks.length > 0 ? anchoredChunks : scoredChunks;
  const minimumRelevantScore = Math.max(6, Math.round(candidateChunks[0].score * 0.35));
  const filteredChunks = candidateChunks.filter((item) => item.score >= minimumRelevantScore);
...
module.exports = {
  ...
  selectLexicalKnowledgeChunks: selectRelevantKnowledgeChunks,
